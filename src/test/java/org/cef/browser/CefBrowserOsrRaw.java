...
public class CefBrowserOsrRaw extends CefBrowser_N implements CefRenderHandler
{

    private CefRenderer renderer;
    ...
    public CefBrowserOsrRaw(CefClient client, String url, boolean transparent, CefRequestContext context, CefBrowserSettings settings)
    {
        this(client, url, transparent, context, null, null, settings);
    }
    private CefBrowserOsrRaw(CefClient client, String url, boolean transparent, CefRequestContext context, CefBrowserOsrRaw parent, Point inspectAt, CefBrowserSettings settings)
    {
        super(client, url, context, parent, inspectAt, settings);
        this.isTransparent = transparent;
        this.settings = settings;
        this.renderer = new CefRenderer(transparent);
        this.createGLCanvas();
    }
    ...
    {
        return new CefBrowserOsrRaw(client, url, this.isTransparent, context, this, inspectAt, this.settings);
    }
    ...
    {
        this.canvas = new GLCanvas(new GLCapabilities(GLProfile.getMaxFixedFunc(true)))
        {
            ...
            {
                CefBrowserOsrRaw.this.createBrowserIfRequired(true);
                if (g instanceof Graphics2D)
                {
                    Graphics2D g2d = (Graphics2D)g;
                    GraphicsConfiguration config = g2d.getDeviceConfiguration();
                    CefBrowserOsrRaw.this.depth = config.getColorModel().getPixelSize();
                    CefBrowserOsrRaw.this.depth_per_component = config.getColorModel().getComponentSize()[0];
                    ...
                    {   // This fixes a weird thing on MacOS: the scale factor being read from getTransform().getScaleX() is incorrect for Java 8 VMs; it is always
                        ...
                        try
                        {
                            ...
                            CefBrowserOsrRaw.this.scaleFactor = factor instanceof Integer
                                ? ((Integer)factor).doubleValue()
                                : 1.0;
                        }
                        catch (ReflectiveOperationException | IllegalArgumentException | SecurityException exc)
                        {
                            CefBrowserOsrRaw.this.scaleFactor = 1.0;
                        }
                    }
                    else
                        CefBrowserOsrRaw.this.scaleFactor = g2d.getTransform().getScaleX();
                }
                super.paint(g);
            }
            @Override
            public void addNotify()
            {
                ...
                if (!this.removed_)
                    return;
                CefBrowserOsrRaw.this.notifyAfterParentChanged();
                this.removed_ = false;
            }
            @Override
            public void removeNotify()
            {
                if (!this.removed_)
                {
                    if (!CefBrowserOsrRaw.this.isClosed())
                        CefBrowserOsrRaw.this.notifyAfterParentChanged();
                    this.removed_ = true;
                }
                super.removeNotify();
        ...
        this.canvas.addGLEventListener(new GLEventListener()
        {
            ...
            public void reshape( GLAutoDrawable glautodrawable, int x, int y, int width, int height)
            {
                ...
                if (OS.isMacintosh())
                {
                    ...
                    newWidth = (int) (width / CefBrowserOsrRaw.this.scaleFactor);
                    newHeight = (int) (height / CefBrowserOsrRaw.this.scaleFactor);
                }
                CefBrowserOsrRaw.this.browser_rect.setBounds(x, y, newWidth, newHeight);
                CefBrowserOsrRaw.this.screenPoint = CefBrowserOsrRaw.this.canvas.getLocationOnScreen();
                CefBrowserOsrRaw.this.wasResized(newWidth, newHeight);
            }
            @Override
            public void init(GLAutoDrawable glautodrawable)
            {
                CefBrowserOsrRaw.this.renderer.initialize(glautodrawable.getGL().getGL2());
            }
            @Override
            public void dispose(GLAutoDrawable glautodrawable)
            {
                CefBrowserOsrRaw.this.renderer.cleanup(glautodrawable.getGL().getGL2());
            }
            @Override
            public void display(GLAutoDrawable glautodrawable)
            {
                CefBrowserOsrRaw.this.renderer.render(glautodrawable.getGL().getGL2());
            }
        });
        this.canvas.addMouseListener(new MouseListener()
        {
            ...
            {
                CefBrowserOsrRaw.this.sendMouseEvent(e);
            }
            @Override
        ...
        this.canvas.addMouseWheelListener(new MouseWheelListener()
        {
            ...
            {
                CefBrowserOsrRaw.this.sendMouseWheelEvent(e);
            }
        });
        this.canvas.addKeyListener(new KeyListener()
        {
            ...
            {
                CefBrowserOsrRaw.this.sendKeyEvent(e);
            }
            @Override
        ...
        this.canvas.addFocusListener(new FocusListener()
        {
            ...
            {
                CefBrowserOsrRaw.this.setFocus(false);
            }
            @Override
            public void focusGained(FocusEvent e)
            {   // Dismiss any Java menus that are currently displayed.
                MenuSelectionManager.defaultManager().clearSelectedPath();
                CefBrowserOsrRaw.this.setFocus(true);
            }
        });
    ...
    public boolean startDragging(CefBrowser browser, CefDragData dragData, int mask, int x, int y) {
        ...
        DragSource.getDefaultDragSource().startDrag(
            ...
            new StringSelection(dragData.getFragmentText()), new DragSourceAdapter()
            {
                ...
                {
                    CefBrowserOsrRaw.this.dragSourceEndedAt(dsde.getLocation(), mask);
                    CefBrowserOsrRaw.this.dragSourceSystemDragEnded();
                }
            }
        );
        return true;
    ...
    public CompletableFuture<BufferedImage> createScreenshot(boolean nativeResolution)
    {
        ...
        {   // If called from another thread, register a GLEventListener and trigger an async redraw, during which we use the GL API to grab the pixel data. An unresolved Future is returned, on which the
            ...
            this.canvas.addGLEventListener(new GLEventListener()
            {
                ...
                {
                    CefBrowserOsrRaw.this.canvas.removeGLEventListener(this);
                    try
                    {
